...
const ReporteDocumentacion = () => {
  ...
  useEffect(() => {
    async function validarYFiltar() {
      try {
        // 1. Validar Seguridad de Director
  ...
  if (checking || loading) return (
    <div className="h-screen flex flex-col items-center justify-center">
      ...
      <p className="font-black text-gray-400 uppercase text-[10px] tracking-widest">Auditoría de Legajos en curso...</p>
    </div>
  );
    ...
    <div className="min-h-screen p-6 md:p-10 bg-[#fcfaf7]">
        ...
        <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            ...
            <p className="text-orange-500 text-[10px] font-black uppercase tracking-[0.2em] mt-2 flex items-center gap-2">
              <AlertCircle size={14}/> {alumnosIncompletos.length} alumnos con documentación pendiente
            </p>
          </div>
          <div className="bg-white px-6 py-4 rounded-2xl border border-gray-100 shadow-sm">
             <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Estado del Archivo</p>
             <p className="text-xl font-black text-[#1a3a5f] uppercase tracking-tight">Santa Catalina v1.8</p>
          </div>
        </header>

        <div className="grid grid-cols-1 gap-6">
          {alumnosIncompletos.map(a => (
            <div key={a.id} className="bg-white p-8 rounded-4xl border border-orange-100 shadow-sm hover:shadow-md transition-all flex flex-col lg:flex-row justify-between gap-8">
              <div className="flex-grow">
                <div className="flex items-center gap-4 mb-4">
                  <div className="bg-orange-50 p-4 rounded-2xl text-orange-500 shadow-sm"><User size={24}/></div>
                  <div>
                ...
                {/* GRILLA DE FALTANTES POR CATEGORÍA */}
              ...
              <div className="flex flex-col justify-center gap-3">
                <button 
                  ...
                  className="bg-gray-800 text-white px-8 py-4 rounded-2xl font-black uppercase text-[9px] tracking-widest hover:bg-[#84bd00] transition-all flex items-center justify-center gap-2 shadow-lg shadow-gray-200"
                >
                  Regularizar Legajo
                </button>
          ...
          {alumnosIncompletos.length === 0 && (
            <div className="bg-green-50/50 p-24 rounded-5xl border-4 border-dashed border-green-100 text-center animate-fade-in">
              ...
              <p className="text-[11px] font-bold text-[#84bd00] uppercase mt-3 tracking-[0.3em]">
                Todos los legajos v1.8 cumplen con los requisitos legales y clínicos.
              </p>
            </div>
          )}
        </div>
      ...
      <footer className="mt-20 py-10 text-center opacity-30">
        <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.3em]">
          auditoría restringida <span className="mx-2 text-orange-500">|</span> s.a.g.a 2026
        </p>
      </footer>
    </div>
  );
