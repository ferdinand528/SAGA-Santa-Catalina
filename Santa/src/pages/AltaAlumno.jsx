...
const AltaAlumno = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Legajo Integral v1.5 registrado con éxito.");
      navigate('/legajos');
    } catch (err) {
  ...
  // Componente Reutilizable para Documentos
  const DocCheck = ({ label, campo, color = "text-gray-500" }) => (
    <label className="flex items-center gap-3 p-3 bg-white rounded-xl border border-gray-100 cursor-pointer hover:bg-green-50 transition-colors group">
      <input 
        type="checkbox" 
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="bg-white/90 backdrop-blur-xl rounded-4xl shadow-2xl border border-white/50 overflow-hidden">
          <div className="bg-[#84bd00] p-8 text-white flex items-center gap-4">
            <UserPlus size={32} />
            <div>
              <h2 className="text-2xl font-black uppercase tracking-tighter leading-none">Alta de Alumno - ver 1.5</h2>
              <p className="text-white/80 text-[10px] font-black uppercase tracking-widest mt-2">I.A.D. Santa Catalina • Gestión Profesional</p>
            </div>
          </div>

          <div className="p-10 space-y-12">
            
            {/* SECCIÓN 1: ALUMNO Y TUTOR */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <input required placeholder="Nombre" className="p-4 bg-gray-50 rounded-2xl font-bold text-sm outline-none" onChange={e => setForm({...form, nombre: e.target.value})} />
                  <input required placeholder="Apellido" className="p-4 bg-gray-50 rounded-2xl font-bold text-sm outline-none" onChange={e => setForm({...form, apellido: e.target.value})} />
                  ...
                  <div className="space-y-1">
                    ...
                    <input type="date" required className="w-full p-4 bg-gray-50 rounded-2xl font-bold text-sm text-gray-500" onChange={e => setForm({...form, fecha_nacimiento: e.target.value})} />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <input required placeholder="Nombre del Tutor" className="w-full p-4 bg-gray-50 rounded-2xl font-bold text-sm outline-none" onChange={e => setForm({...form, tutor_nombre: e.target.value})} />
                <div className="grid grid-cols-2 gap-4">
                  <input required placeholder="Celular" className="p-4 bg-gray-50 rounded-2xl font-bold text-sm outline-none" onChange={e => setForm({...form, tutor_celular: e.target.value})} />
                  <input required placeholder="Domicilio" className="p-4 bg-gray-50 rounded-2xl font-bold text-sm outline-none" onChange={e => setForm({...form, tutor_domicilio: e.target.value})} />
                </div>
              </div>
            </div>

            {/* SECCIÓN 2: CHECKLIST DE DOCUMENTACIÓN EXPANDIDO */}
            <div className="p-8 bg-gray-50/50 rounded-3xl border border-gray-100">
              <div className="flex items-center gap-2 mb-6 border-b border-gray-200 pb-2">
                <FileCheck size={16} className="text-[#84bd00]"/>
                <h3 className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Control de Documentación y Permisos</h3>
              </div>
            ...
            {/* SECCIÓN 3: ADMINISTRACIÓN Y SALUD */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="p-8 bg-blue-50/30 rounded-3xl border border-blue-100 space-y-4">
                <h3 className="text-[10px] font-black text-blue-600 uppercase tracking-widest flex items-center gap-2"><CreditCard size={14}/> Convenio y Obra Social</h3>
                <div className="grid grid-cols-1 gap-4">
                  ...
                  {form.obra_social === 'OTRA' && (
                    <input required placeholder="Especificar Obra Social" className="p-4 bg-white rounded-2xl font-bold text-sm border-2 border-[#84bd00]/30 outline-none animate-fade-in" onChange={e => setOtraObraSocial(e.target.value.toUpperCase())} />
                  )}
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[8px] font-black text-red-500 uppercase ml-2">Venc. CUD</label>
                      <input type="date" className="w-full p-4 bg-white rounded-2xl font-bold text-sm text-gray-500 border border-red-100" onChange={e => setForm({...form, fecha_vencimiento_cud: e.target.value})} />
                    </div>
                    <div className="space-y-1">
              ...
              <div className="space-y-4">
                <h3 className="text-[10px] font-black text-red-500 uppercase tracking-widest flex items-center gap-2"><Activity size={14}/> Ficha de Salud Inicial</h3>
                <textarea placeholder="Diagnóstico / Patología" rows="3" className="w-full p-4 bg-red-50/20 border border-red-50 rounded-2xl font-bold text-sm outline-none" onChange={e => setForm({...form, patologia: e.target.value})} />
                <textarea placeholder="Medicación y Frecuencia" rows="3" className="w-full p-4 bg-blue-50/20 border border-blue-50 rounded-2xl font-bold text-sm outline-none" onChange={e => setForm({...form, medicacion: e.target.value})} />
              </div>
            </div>

            <button disabled={loading} className="w-full bg-[#84bd00] text-white p-6 rounded-3xl font-black uppercase tracking-widest shadow-xl hover:bg-black transition-all flex items-center justify-center gap-3">
              {loading ? <Loader2 className="animate-spin" /> : <Save size={20}/>}
              {loading ? "PROCESANDO..." : "REGISTRAR LEGAJO INTEGRAL v1.5"}
            </button>
          </div>
        </form>
      </div>
